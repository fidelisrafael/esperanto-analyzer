...
class CLI():
    ...
    @staticmethod
    def display_output_for_analyzer(analyzer):
        if analyzer.processed is False:
            raise 'Analyzer must be processed before output display. You must call `analyze()` in your instance'

        CLI.print_results(analyzer.results())
    ...
    def format_table_data(results):
        ...
        for data in results:
            ...
            out_data.append([format_color(data[0], pos_name), format_color(pos_name, pos_name)])

        return out_data

    @staticmethod
    def print_results(results, width=15):
        table_data = CLI.format_table_data(results)

        return tableprint.table(table_data, CLI.OUTPUT_TABLE_HEADERS, width=width)
